...
export async function Enhance(item: Item, enhancer: Item): Promise<Item | null> {
	const userId = get(store.user)?.id;
	...
	const res = await fetch(`/api/item/${userId}/enhance`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify({ dbItem: encodeDbItem(item), dbEnhancer: encodeDbItem(enhancer) })
	});
