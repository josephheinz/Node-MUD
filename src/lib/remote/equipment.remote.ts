...
import { EmptyEquipment, Equipment, Rarity, type DBEquipment, type DBInventory, type DBItem, type EquipmentSlot, type IItemModifier, type Item } from "$lib/types/item";
...
const RawModifierSpecSchema = z.object({
    type: z.string(),
}).catchall(z.any());
...
export const equip = form(equipRequestSchema, async ({ id, dbItem }) => {
    const { locals } = getRequestEvent();
...
export const unequip = form(unequipRequestSchema, async ({ id, slot }) => {
    const { locals } = getRequestEvent();
    const user = locals.user;
