...
export async function POST({ request, params, cookies }) {
	...
	if (equipment[slot]) {
		...
		// revive all modifiers so they have .hash()
		equippedItem.modifiers = reviveModifiers(equippedItem.modifiers);
		inventory.push(encodeDbItem(equippedItem));
	}

	// Equip new item
	equipment[slot] = loadDbItem(encodeDbItem(item));
