...
export async function POST({ request, params, cookies }) {
    ...
    if (index === -1) {
        ...
        slot = (Object.entries(equipment)
            .find(([_, i]) => i?.id === dbItem.id)?.[0]) as keyof Equipment | undefined;

    }

    if (!slot && index === -1) return new Response(JSON.stringify({ error: "Item not in inventory or equipped" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
    });
    ...
    item.modifiers.push(instantiateModifierFromClass(new ReforgeModifier(reforge.name)));
    ...
    if (index === -1 && slot) {
        ...
        if (updateError || !updateData) {
            return new Response(JSON.stringify({ error: updateError?.message || "Failed to update equipment" }), {
                status: 500,
                headers: { "Content-Type": "application/json" }
            });
        }
        ...
        if (updateError || !updateData) {
            return new Response(JSON.stringify({ error: updateError?.message || "Failed to update inventory" }), {
                status: 500,
                headers: { "Content-Type": "application/json" }
            });
        }
